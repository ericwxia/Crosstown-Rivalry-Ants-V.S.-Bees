...
class Place:
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant():
            ...
            if self.ant is None:
                ...
                self.ant = insect
            elif not insect.can_contain(self.ant) and not self.ant.can_contain(insect):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant():
            ...
            if type(insect) == QueenAnt and insect.index == 1:
                return
            elif type(insect) == BodyguardAnt and insect.ant is not None:
                self.ant = insect.ant
            else:
                self.ant = None
        else:
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        anant = self.place.ant
        if anant is None or (anant is not None and anant.blocks_path == False):
            return False
        else:
            return True
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        location = self.place
        index = 0
        while location != hive or location.entrance != None:
            if index <= self.max_range and index >= self.min_range and len(location.bees) != 0:
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        if self.digesting > 0:
            self.digesting -= 1
        elif self.digesting == 0 and len(self.place.bees) != 0:
            bees = self.place.bees
...
class QueenAnt(ThrowerAnt):
    ...
    def __init__(self):
        ...
        self.index = self.number_of_queens  # index is a static counter; starts from 1
    ...
    def action(self, colony):
        ...
        if self.index > 1:
            ...
            while ant_place:
                if ant_place.ant:
                    if type(ant_place.ant) == BodyguardAnt:
                        if ant_place.ant.ant:
                            ant_place.ant.damage = type(ant_place.ant.ant).damage * 2
                    else:
                        ant_place.ant.damage = type(ant_place.ant).damage * 2
                ant_place = ant_place.exit

class QueenPlace(object):
    def __init__(self,  colony_queen, place):
